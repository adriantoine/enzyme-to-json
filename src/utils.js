...
export const isShallowWrapper = wrapper =>
  wrapper && wrapper.constructor && wrapper.constructor.name === SHALLOW_WRAPPER_NAME;
export const isReactWrapper = wrapper =>
  wrapper && wrapper.constructor && wrapper.constructor.name === REACT_WRAPPER_NAME;
export const isCheerioWrapper = wrapper => wrapper && wrapper.cheerio;

export const isEnzymeWrapper = wrapper =>
  isShallowWrapper(wrapper) || isReactWrapper(wrapper) || isCheerioWrapper(wrapper);

export const compact = array => filter(array, item => !isNil(item) && item !== '');
